...
module "bigip_1" {
  ...
  labels = local.cfe_labels
  INIT_URL = "https://github.com/F5Networks/f5-bigip-runtime-init/releases/download/1.5.0/f5-bigip-runtime-init-1.5.0-1.gz.run"
# Here we are overriding the module defaults to use the latest versions
  DO_URL = "https://github.com/F5Networks/f5-declarative-onboarding/releases/download/v1.30.0/f5-declarative-onboarding-1.30.0-3.noarch.rpm"
  AS3_URL = "https://github.com/F5Networks/f5-appsvcs-extension/releases/download/v3.37.0/f5-appsvcs-3.37.0-4.noarch.rpm"
  CFE_URL = "https://github.com/F5Networks/f5-cloud-failover-extension/releases/download/v1.11.0/f5-cloud-failover-1.11.0-0.noarch.rpm"
  TS_URL = "https://github.com/F5Networks/f5-telemetry-streaming/releases/download/v1.29.0/f5-telemetry-1.29.0-1.noarch.rpm"
...
# In these blocks we write out the JSON files per student for each lab
resource "local_file" "DO" {
count = 2
  content = templatefile("./templates/do.json",{
    bigip_admin = module.bigip_1.f5_username
    bigip_admin_password = format("%s%s",module.bigip_1.bigip_password,var.student_id)
    bigip2_admin = module.bigip_2.f5_username
    bigip2_admin_password = format("%s%s",module.bigip_2.bigip_password,var.student_id)
  ...
  filename = format("./ATC_Declarations/Lab4.1-DO/do_BIGIP%d.json", count.index+1)
}
resource "local_file" "TS" {
  content = templatefile("./templates/ts.json",{
    client_email = jsonencode(jsondecode(module.service_accounts.keys["bigip"])["client_email"])
    project_id = jsonencode(jsondecode(module.service_accounts.keys["bigip"])["project_id"])
  })
  filename = format("./ATC_Declarations/Lab4.2-TS/ts.json")
}      
...
count = 2
  ...
  filename = format("./ATC_Declarations/Lab4.3-DO_HA/do_HA_BIGIP%d.json", count.index+1)
}
...
resource "local_file" "AS3" {
  ...
  external_address = google_compute_forwarding_rule.fwd.ip_address
  student_id = var.student_id
  })
  filename = format("./ATC_Declarations/Lab4.5-AS3/as3.json")
}

# Write file for optional exercise      
#***Removing as this is not relevant to the general flow of the lab. CARLB
...
#  student_id = var.student_id
#  })
#  filename = format("./ATC_Declarations/Lab4.3-AS3/as3_Optional.json")
#}
    
resource "google_compute_target_instance" "target_1" {
  project     = var.project_id
  name        = format("student%d-bigip1", var.student_id)
  zone        = module.bigip_1.zone
  instance    = module.bigip_1.self_link
}

  resource "google_compute_target_instance" "target_2" {
  project     = var.project_id
  name        = format("student%d-bigip2", var.student_id)
  zone        = module.bigip_2.zone
  instance    = module.bigip_2.self_link
}
  
#CFE Forwarding rule
resource "google_compute_forwarding_rule" "fwd" {
  project               = var.project_id
  name                  = format("student%d", var.student_id)
  region                = var.region
  ip_protocol           = "TCP"
  all_ports             = true
  load_balancing_scheme = "EXTERNAL" 
  labels                = local.cfe_labels
  target                = google_compute_target_instance.target_1.self_link
}
